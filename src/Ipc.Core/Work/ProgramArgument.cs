...
public sealed class ProgramArgument
{
    ...
    public ProgramArgument(Func<object?> read, Action<object?> write, Func<object?, object?> normalize,
        string? type = null, int length = 0, int decimals = 0, Func<ProgramBuffer>? buffer = null)
    { _read = read; _write = write; _normalize = normalize; Type = type; Length = length; Decimals = decimals; _buffer = buffer; }
